...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="wedding-sans min-h-screen bg-linear-to-br from-[#fff2f2] via-[#FDFDFC] to-[#F3BEC7]/35 dark:from-[#1D0002] dark:via-[#0a0a0a] dark:to-[#4B0600]/25 text-[#1b1b18] flex items-center justify-center px-6 py-10">
        <div class="relative w-full max-w-4xl">
            {{-- Background decoration --}}
            <div aria-hidden="true" class="pointer-events-none absolute inset-0 overflow-hidden rounded-[1.25rem]">
                <div class="absolute -top-10 -left-10 w-[260px] h-[260px] rounded-full bg-[#fff2f2] dark:bg-[#1D0002] wedding-float" style="animation-delay: 0.1s;"></div>
                <div class="absolute top-24 -right-10 w-[220px] h-[220px] rounded-full bg-[#fff2f2] dark:bg-[#1D0002] wedding-float" style="animation-delay: 1.3s;"></div>

                <span class="absolute top-[16%] left-[8%] text-[#F8B803] dark:text-[#391800] wedding-float text-2xl" style="animation-duration: 6.2s;">&#10084;</span>
                <span class="absolute top-[42%] right-[12%] text-[#F53003] dark:text-[#FF4433] wedding-float text-xl" style="animation-duration: 5.2s; animation-delay: 0.7s;">&#10084;</span>
                <span class="absolute bottom-[18%] left-[16%] text-[#F3BEC7] dark:text-[#4B0600] wedding-float text-2xl" style="animation-duration: 6.8s; animation-delay: 0.4s;">&#10084;</span>
                <span class="absolute top-[30%] right-[40%] text-[#F3BEC7] dark:text-[#4B0600] wedding-float text-xl" style="animation-duration: 7.2s; animation-delay: 1.1s;">&#10084;</span>

                <span class="absolute top-[8%] right-[18%] text-[#F3BEC7] dark:text-[#4B0600] wedding-float text-lg" style="animation-duration: 6.5s; animation-delay: 0.2s;">&#10084;</span>
                <span class="absolute top-[58%] left-[10%] text-[#F8B803] dark:text-[#391800] wedding-float text-lg" style="animation-duration: 7.5s; animation-delay: 0.9s;">&#10084;</span>
                <span class="absolute top-[26%] left-[22%] text-[#F53003] dark:text-[#FF4433] wedding-float text-lg" style="animation-duration: 5.9s; animation-delay: 1.4s;">&#10084;</span>
                <span class="absolute bottom-[12%] right-[22%] text-[#F8B803] dark:text-[#391800] wedding-float text-xl" style="animation-duration: 6.9s; animation-delay: 0.6s;">&#10084;</span>
                <span class="absolute top-[36%] left-[44%] text-[#F3BEC7] dark:text-[#4B0600] wedding-float text-2xl" style="animation-duration: 8.2s; animation-delay: 1.8s;">&#10084;</span>
                <span class="absolute top-[64%] right-[36%] text-[#F53003] dark:text-[#FF4433] wedding-float text-lg" style="animation-duration: 6.3s; animation-delay: 0.1s;">&#10084;</span>
            </div>
            ...
            <main class="relative">
                <section class="wedding-fade wedding-hero px-6 py-8 lg:px-10 lg:py-10 overflow-hidden text-center" data-animate>
                    {{-- Shimmer stripe --}}
                    <div aria-hidden="true" class="absolute top-0 left-0 w-full h-full pointer-events-none">
                        <div class="absolute -top-24 left-[-20%] w-[45%] h-[200px] bg-white/10 dark:bg-white/5 rotate-[8deg]" style="animation: wedding-shimmer 3.2s ease-in-out infinite;"></div>
                    </div>
                            ...
                            @foreach ($media as $item)
                                <figure class="relative overflow-hidden group">
                                    <img
                                        ...
                                        loading="lazy"
                                    >
                                    <figcaption class="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 flex items-center justify-center">
                ...
                <section id="wishes" class="mt-10 wedding-fade rounded-[1.25rem] bg-white/85 dark:bg-[#161615]/85 backdrop-blur-sm shadow-sm px-6 py-8 lg:px-10" data-animate>
                    <div class="mb-6">
                    ...
                    <div id="wishToast" class="hidden mt-5 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#161615] px-4 py-3 text-sm text-gray-700 dark:text-gray-200" role="status" aria-live="polite"></div>
                </section>
            </main>
        </div>
        ...
        <script>
            (function () {
                ...
                if ('IntersectionObserver' in window) {
                    ...
                    animated.forEach(function (el) { el.classList.add('wedding-in-view'); });
                }
                ...
                function renderWishes() {
                    ...
                    if (latest.length === 0) {
                        list.innerHTML = '<div class="text-sm text-gray-600 dark:text-gray-400">No wishes yet. Be the first.</div>';
                        return;
                    }
                    ...
                    list.innerHTML = latest.map(function (wish) {
                        ...
                        return [
                            ...
                            '  <div class="flex items-center justify-between gap-4">',
                            '    <div class="text-sm font-semibold text-gray-800 dark:text-gray-200">' + name + '</div>',
                            '    <div class="text-xs text-gray-500 dark:text-gray-400">' + time + '</div>',
                            '  </div>',
                            title ? '  <div class="mt-1 text-sm font-medium text-gray-700 dark:text-gray-300">' + title + '</div>' : '',
                            '  <p class="mt-2 text-sm text-gray-700 dark:text-gray-200 whitespace-pre-wrap">' + message + '</p>',
                            '</article>'
                        ].join('');
                    }).join('');
                }
                ...
                if (form) {
                    form.addEventListener('submit', function (e) {
                        ...
                        var name = document.getElementById('wishName') ? document.getElementById('wishName').value.trim() : '';
                        var title = document.getElementById('wishTitle') ? document.getElementById('wishTitle').value.trim() : '';
                ...
                renderWishes();
                animated.forEach(function (el) { el.classList.add('wedding-fade'); });
            })();
        </script>
    </body>
</html>
